...
PatternSmith is a fluent regex builder and pattern intelligence engine that transforms how developers write regular expressions. Instead of cryptic syntax, developers chain semantic methods like forge().startOfLine().digits(3).literal("-").digits(4).endOfLine().build(). Features: (1) Fluent chainable API with 20+ semantic methods including named captures, lookaheads, alternation, quantifiers, and sub-builder composition with input validation and descriptive TypeErrors. (2) Pattern library with 12 production-ready validators (email RFC 5322, URL, IPv4 with octet validation, phone, ISO date, hex color, semver, UUID v4, JWT, credit card, slug, strong password). (3) Human-readable pattern explainer that tokenizes any regex into plain-English descriptions. (4) Anti-pattern detector identifying catastrophic backtracking (ReDoS), greedy traps, empty alternation, suspicious ranges with severity levels and risk assessment. (5) Pattern optimizer with actionable improvement suggestions. (6) Test runner with capture groups, named groups, and timing. Clean code with descriptive variable names, JSDoc with @example blocks, and comprehensive error handling.
